...
<!-- SHOW THE SERVICES OF THAT SHOP -->
        <h4 style="text-align:center">Shop ID:<?php echo $_GET['id'] ?></h4></br>
        <h4 style="text-align:center">SERVICE(s)</h4>
        <div style="margin-left:50px; margin-right:50px;  margin-bottom:50px;text-align:center;background:lightblue;font-size:larger">
            <div class="row" style="padding:5px;">
                <div class="col-md-5" style="color:indigo;font-weight:bold"> Service Name</div>
                <div class="col-md-5" style="color:indigo;font-weight:bold"> Charge(BDT)</div>
            </div><hr>


<!-- PHP CODE & SQL COMMANDS SHOW THE SERVICES -->
            <?php
            $shop_id = $_GET['id'];
            $sql = "SELECT shops.shop_id,offered_services.service_name,offered_services.service_cost FROM `offered_services`INNER JOIN shops ON shops.shop_id=offered_services.shop_id where offered_services.shop_id=$shop_id;";
            ...
            ?>
                    <div class="row" style="padding:5px;;text-align:center">
                        ...
                        <div class="col-md-5"><?php echo $charge=number_format($row[2],0); ?></div>
...
<h4 style="text-align:center">HISTORY</h4>
        <div style="margin-left:50px; margin-right:50px;  margin-bottom:50px;text-align:center;background:lightskyblue;font-size:larger">
            <div class="row" style="padding:5px;">
                ...
                <div class="col-md-1" style="color:indigo;font-weight:bold"> Customer ID</div>
...
<!-- PHP CODE & SQL COMMANDS SHOW THE HISTORY -->
            <?php
            $shop_id = $_GET['id'];
            $sql = "SELECT * FROM payment_details WHERE shop_id=$shop_id";
...
<!-- SHOW THE REVIEW OF THAT SHOP -->
        ...
        <div style="margin-left:50px; margin-right:50px;  margin-bottom:50px;text-align:center;background:lavenderblush;font-size:larger">
            <div class="row" style="padding:5px;">
                <div class="col-md-5" style="color:indigo;font-weight:bold"> Customer Name</div>
...
<!-- PHP CODE & SQL COMMANDS SHOW THE REVIEWS -->
            <?php
            $shop_id = $_GET['id'];
            $sql = "SELECT reviews.customer_id,user.name,reviews.review FROM reviews INNER JOIN user on user.user_id=reviews.customer_id WHERE shop_id=$shop_id";
            ...
            ?>
                    <div class="row" style="padding:5px;;text-align:center">
                        <div class="col-md-5"><?php echo $row[1]; ?></div>
                        <div class="col-md-5" style="text-transform:capitalize"><?php echo $row[2]; ?></div>
        ...
        <p style="text-align:center"><a href="adminview.php?id=<?php echo $_GET['id']; ?>">
            <button style="background-color:brown;border:0cm; border-radius: 5px;color: white;">Delete This Shop</button></a>
        </p>
