  ...
  <body>
    <header>
      <nav class="navbar navbar-expand-lg navbar-dark" style="background-color: #398D7F;">
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-5 mx-auto">
                        ...
                        <form role="form" enctype="multipart/form-data" method="post" action="changepassword.php">
                                <div class="form-group">
                                    <label style="font-size: larger;">Current Password</label> 
                                    <input type="int" class="form-control" name="old_pass" >
                                </div></br>
                                <div class="form-group">
                                    <label style="font-size: larger;">New Password</label> 
                                    <input type="text" class="form-control" name="new_pass">
                                </div></br>
                                <div class="form-group">
                                    <label style="font-size: larger;">Confirm New Password</label> 
                                    <input type="int" class="form-control" name="cnew_pass" >
                                </div></br>
        ...
        <?php
        if(isset($_POST['old_pass']) && isset($_POST['new_pass']) && isset($_POST['cnew_pass'])){
        $op = $_POST['old_pass'];
        $np = $_POST['new_pass'];
        $cnp = $_POST['cnew_pass'];
        if($np==$cnp)
            {
            $query="select * from user where password='$op' ";
            $result=mysqli_query($conn,$query);
            $mehedi=mysqli_num_rows($result);
            if ($mehedi!=0)
            {
                $query="update user set password='$np' where password='$op'";
                $result=mysqli_query($conn,$query);
